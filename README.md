...
For a one-click solution not relying on the package's ```setup.py```, users can try running a shell script command such as  ```virtualenv .env && source .env/bin/activate && pip install -r requirements.txt```
